...
async function check_status() {
    ...
    if(is_id_proof) 
        ...
        $(".ubi_not_kyc").show();
    const {data: veriff} = await axios.get(`https://www.jax.money:8443/veriff/user/${publicKey}`);
    if(publicKey != accounts[0]) return;
    if(veriff.type == 'success') {
        if(veriff.user.publicKey != publicKey.toLowerCase()){
            $(".btn_verify").html("&nbsp");
            return;
        }
        if(veriff.status == "") { return; }
        switch(veriff.status) {
            case 'approved':
                check_user_info(publicKey);
                return;
            case 'submitted':
                $(".ubi_not_kyc").hide();
                $(".ubi_id_submitted").show();
...
async function check_user_info(publicKey) {
